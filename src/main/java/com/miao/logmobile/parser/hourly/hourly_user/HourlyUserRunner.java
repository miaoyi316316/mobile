...
public class HourlyUserRunner implements Tool {


    private static final Logger logger = Logger.getLogger(HourlyUserRunner.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            int status = ToolRunner.run(new HourlyUserRunner(), args);
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "hourly_active_user");

        job.setJarByClass(getClass());

        job.setNumReduceTasks(4);

        job.setMapperClass(HourlyUserMapper.class);
        ...
        job.setReducerClass(HourlyUserReducer.class);
